export const PRODUCTS = [
    {
        id: 1, 
        categoryId: 1, 
        description: 'Esta tablet Samsung es la compañera ideal, con capacidad de sobra para cada una de tus actividades. El diseño delgado, compacto y portátil, con facilidad para sostener en una mano, lo convierte en una combinación perfecta de rendimiento y versatilidad.', 
        image: 'https://http2.mlstatic.com/D_NQ_NP_765874-MLA47146197629_082021-O.webp', 
        name: 'Tablet Samsung Galaxy Tab A7 Lite SM-T220 8.7" 32GB', 
        price: 37.999, 
        stock: 10
    },
    {
        id: 2, 
        categoryId: 2, 
        description: 'Samsung es reconocida a nivel mundial como una empresa líder en la industria tecnológica. Todos sus productos son diseñados con una calidad superior y pensados para contribuir a un futuro mejor. Por eso, va a hacer que disfrutes de una experiencia visual incomparable.', 
        image: 'https://http2.mlstatic.com/D_NQ_NP_787221-MLA48007684849_102021-O.webp', 
        name: 'Smart TV Samsung Series 7 UN50AU7000GCZB LED 4K 50', 
        price: 111.999, 
        stock: 6
    },
    {
        id: 3, 
        categoryId: 3, 
        description: 'Incluyen una plantilla SoftFoam+ y una media suela de IMEVA moldeada por inyección. No solo disfrutarás de una amortiguación cómoda todo el día, sino que también reducirás tu huella de carbono.', 
        image: 'https://http2.mlstatic.com/D_NQ_NP_725347-MLA50332740995_062022-O.webp', 
        name: 'Zapatilla Puma Original X-ray2 Square Better Unisex', 
        price: 20.899, 
        stock: 55
    },
]